...
export type RemovePathAnalyticsInput = {
  repoKey: string;
  targetPath: string;
  pathType: 'file' | 'directory';
};

export type RemovePathAnalyticsResult = {
  deleted: {
    pathStatuses: number;
    history: number;
    suggestions: number;
...
function pathInSubtree(candidate: string, target: string, pathType: 'file' | 'directory'): boolean {
...
function buildPathMatchQuery(repoKey: string, targetPath: string, pathType: 'file' | 'directory') {
...
function buildSuggestionMatchQuery(repoKey: string, targetPath: string, pathType: 'file' | 'directory') {
  const fp = normalizePath(targetPath);
...
function buildModuleSnapshotMatchQuery(repoKey: string, targetPath: string, pathType: 'file' | 'directory') {
  const fp = normalizePath(targetPath);
...
function collectAncestorsForTarget(targetPath: string, pathType: 'file' | 'directory'): string[] {
  const fp = normalizePath(targetPath);
  if (pathType === 'file') {
...
export async function removePathAnalytics(
  input: RemovePathAnalyticsInput,
): Promise<RemovePathAnalyticsResult> {
