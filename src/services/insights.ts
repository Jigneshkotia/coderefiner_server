...
export async function getOverview(repoKey: string) {
  ...
  const moduleScores = latestModules.map((m) => m.doc.healthScore as number | null);
  const pageScores = latestPages.map((p) => p.doc.healthScore as number | null);
  ...
  const hotspots = [
    ...latestModules
      .map((m) => ({ type: 'module' as const, path: m._id as string, healthScore: m.doc.healthScore as number }))
      .filter((h) => h.healthScore != null)
      .sort((a, b) => (a.healthScore ?? 100) - (b.healthScore ?? 100))
      .slice(0, 5),
    ...latestPages
      .map((p) => ({ type: 'page' as const, path: p._id as string, healthScore: p.doc.healthScore as number }))
      .filter((h) => h.healthScore != null)
      .sort((a, b) => (a.healthScore ?? 100) - (b.healthScore ?? 100))
      .slice(0, 5),
  ];
...
export async function getInsights(repoKey: string) {
  ...
  const regressions: { type: string; path: string; delta: number }[] = [];
  const byModule = new Map<string, typeof modules>();
  ...
  for (const [modulePath, snaps] of byModule) {
    if (snaps.length >= 2 && snaps[0].healthScore != null && snaps[1].healthScore != null) {
      ...
      if (delta <= -10) regressions.push({ type: 'module', path: modulePath, delta });
    }
  }
