export interface ParsedSuggestionLocation {
  filePath: string;
  line?: number;
}
...
export function parseSuggestionLocation(location: string): ParsedSuggestionLocation | null {
...
export function suggestionFilePath(location: string, filePath?: string): string | undefined {
  if (filePath?.trim()) {
...
export function normalizeRepoFilePath(p: string): string {
