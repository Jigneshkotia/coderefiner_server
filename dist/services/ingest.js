...
function deriveAuditMode(compileParams, runtimeParams) {
    if (compileParams.length > 0 && runtimeParams.length === 0)
        return 'compile-only';
    if (compileParams.length === 0 && runtimeParams.length > 0)
        return 'runtime-only';
    if (compileParams.length > 0 && runtimeParams.length > 0)
...
function buildSummary(suggestions, moduleCount, pageCount) {
    const byType = {};
    const byImportance = {};
    const byParameter = {};
...
function suggestionsForModule(suggestions, modulePath, files) {
...
function collectIngestFilePaths(payload) {
    const paths = new Set();
    const add = (p) => { if (p)
        paths.add(normalizePath(p)); };
    for (const p of payload.analyzedPaths ?? [])
        add(p);
    for (const p of payload.run?.targetFiles ?? [])
        add(p);
    for (const p of Object.keys(payload.pathLineCounts ?? {}))
        add(p);
...
export async function ingestAnalysis(payload) {
    ...
    await Repo.findOneAndUpdate({ repoKey }, {
        $set: {
            repoKey,
            displayName: repoKey,
            owner,
    ...
    if (payload.isInitialRepoSync && payload.repoTree) {
        await saveRepoTree(repoKey, payload.repoTree.files, payload.repoTree.directories, payload.repoTree.files.length >= 10000);
    }
        ...
        : run.compileParams.length > 0
            ? [{
                    modulePath: run.targetFiles.length === 1
                        ...
                        : '.',
                    moduleScope: run.targetFiles.length === 1 ? 'file' : 'directory',
    ...
    if (analyzedPaths.length > 0) {
        await upsertPathStatuses(repoKey, run.runId, analyzedPaths, suggestions, run.compileParams, run.runtimeParams, pathLineCounts);
        for (const rawPath of analyzedPaths) {
            await appendPathHistory(repoKey, run.runId, analyzedAt, 'file', normalizePath(rawPath), 'file', suggestions, run.compileParams, run.runtimeParams, pathLineCounts);
        }
        if (analysisScope === 'folder-full' && moduleBuckets[0]) {
            await appendPathHistory(repoKey, run.runId, analyzedAt, analysisScope, normalizePath(moduleBuckets[0].modulePath), 'directory', suggestions, run.compileParams, run.runtimeParams, pathLineCounts);
        }
    }
    ...
    for (const bucket of moduleBuckets) {
        ...
        const { paramScores, issueCounts, issuePenalty } = computeParamScores(bucket.compileParamsAudited, modSuggestions);
    ...
    for (const bucket of pageBuckets) {
        ...
        for (const s of pageSuggestions) {
            Object.assign(rawMetrics, s.extractedMetrics ?? extractFromSuggestion(s));
        }
        ...
        let urlPath = '/';
        try {
            urlPath = new URL(bucket.url).pathname;
        }
        catch { /* keep / */ }
        await PageSnapshot.create({
            ...
            rawMetrics,
            cwvRatings: {
                lcp: rateLcp(rawMetrics.lcpMs),
                inp: rateInp(rawMetrics.inpMs),
                cls: rateCls(rawMetrics.cls),
            },
            issueCounts: {},
            healthScore: health,
